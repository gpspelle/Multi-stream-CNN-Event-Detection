...
if __name__ == '__main__':
    argp = argparse.ArgumentParser(description='Do training and result  tasks')
    argp.add_argument("-actions", dest='actions', type=str, nargs='+',
            help='Usage: -actions <train? <cross_train?> <result?>
                  Example: -actions train result
                           -actions cross_train result
                           -actions train
                           -actions result', required=True)
...
worker = Worker(args.classes[0], args.classes[1], args.num_features,
                        args.input_dim[0], args.input_dim[1])

'''
    todo: split worker in train, cross train and result
'''
